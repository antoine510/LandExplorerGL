...
void updateLightSource(LightSource* light, Terrain* terrain)
{
	...
	Uint32* doX = malloc(2 * blocCount * sizeof(Uint32));	//Allow space for current and next set of data
	Uint32* doY = malloc(2 * blocCount * sizeof(Uint32));
	char* intensity = malloc(2 * blocCount);
    ...
    while(pointCount > 0)
    {
        ...
        {
            char nextIntensity = intensity[i] - max(1,getBlockOpacity(terrain, doX[i] + light->x - LIGHT_MASK_SIZE/2, doY[i] + light->y - LIGHT_MASK_SIZE/2));
            if(nextIntensity > 0)
            {
...
SDL_Color getLight(LightSource* light, Uint32 x, Uint32 y)
{
    ...
    return (SDL_Color){(light->color.r * (intensity+1)) >> 4,
                       (light->color.g * (intensity+1)) >> 4,
                       (light->color.b * (intensity+1)) >> 4, intensity};      //The alpha component is the intensity
}
...
void addLightSource(Terrain* terrain, Uint32 x, Uint32 y, SDL_Color color, Uint8 intensity, int xOffset, int yOffset)
{
    ...
    lightSources = realloc(lightSources, (lightIndex+1) * sizeof(LightSource));
