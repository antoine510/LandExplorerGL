...
{
    ChunckRenderer* cRenderer = malloc(sizeof(ChunckRenderer));
...
void setTerrain(ChunckRenderer* chunckRenderer, Terrain* terrain)
{
    ...
    chunckRenderer->textureID = malloc(terrain->widthChunck * terrain->heightChunck * sizeof(char));
...
int createChunck(ChunckRenderer* cRenderer, int x, int y)
{
    ...
    loop(&cRenderer->currentTextureID, MAX_TEXTURE_STORAGE);
