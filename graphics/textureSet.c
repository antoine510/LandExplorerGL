...
{
    TextureSet* texSet = calloc(1, sizeof(TextureSet));
    ...
    while(texture)
    {
        ...
        strcat(fileName, asStringl(texture, "filename"));
        stateTex = createTextureFromFile(renderer, fileName);
...
void setTexSetColorMod(TextureSet* texSet, SDL_Color* color)
{
    ...
    {
        if(texSet->textures[i] != NULL) setTextureColorMod(texSet->textures[i], color);
    }
}
