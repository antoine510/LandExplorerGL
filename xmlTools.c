...
{
    return createDynamicData((Vect2){asFloatl(node, "vx"), asFloatl(node, "vy")});
}

CollisionData getCollisionData(xmlNodePtr node)
{
    return createCollisionData((Vect2){asFloatl(node, "x"), asFloatl(node, "y")},
                               (Vect2){asFloatl(node, "w"), asFloatl(node, "h")},
