...
ModeUpdateResult updateEditor(Editor* editor, Level* level, KeyStates* keyStates)
{
    ...
    case VIEWING:
        ...
        else if(isMouseRightUp())
        {
            ...
            pasteModule(module, level, asPoint(posx, posy), true, false);
    ...
    case SELECTING_MODULE:
        ...
        if(isMouseLeftUp())
        {
            ...
            if(posx < editor->selection.x) exchange(&posx, &editor->selection.x);   //If the rectangle is not legal we exchange corners
            if(posy < editor->selection.y) exchange(&posy, &editor->selection.y);
    ...
    case ENTERING_NAME:
        ...
        else if(isKeyHeld(keyStates, key_backspace))     //We want to erase one char in our module name
        {
            ...
            if((moduleNameLength = strlen(editor->moduleName)) > 0) editor->moduleName[moduleNameLength-1] = '\0';
