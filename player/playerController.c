...
{
    PlayerControl* pControl = malloc(sizeof(PlayerControl));
...
void updateController(KeyStates* keyStates, Level* level, int playerID, PlayerControl* pControl)
{
    ...
    if(frame % (int)PCPS == 0)
    {
        ...
        clampf(&level->entities->status[playerID].HP, 0.0f, MAX_PLAYER_HP);
    }
    frame++;
...
void playerRespawn(Level* level, int playerID)
{
    ...
    level->entities->dynData[playerID].velocity = (Vect2){0.0f, 0.0f};
