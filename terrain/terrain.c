...
Terrain* initTerrain()
{
    Terrain* terrain = calloc(1, sizeof(Terrain));
    if(terrain == NULL) {printf("Error while initialiazing terrain");}

    terrain->width = 1024; terrain->height = 1024;
    ...
    terrain->blocks = calloc(terrain->width * terrain->height, sizeof(Bloc));
    terrain->backwall = calloc(terrain->width * terrain->height, sizeof(Bloc));
    terrain->chuncks = calloc(terrain->widthChunck * terrain->heightChunck, sizeof(Chunck));
...
void setBlocCustomValue(Terrain* terrain, Uint32 x, Uint32 y, Uint8 customValue)
{
    ...
    if(*customValues == NULL)
    {
        ...
        *customValues = calloc(CHUNCK_WIDTH * CHUNCK_HEIGHT, sizeof(Uint8));
    }
