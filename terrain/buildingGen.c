...
{
    const int moduleCount = getRandomInRange(10, 14);
	int xBuilding[MAX_STRUCTURE_BUILDINGS];
	BuildingTypes buildingType[MAX_STRUCTURE_BUILDINGS];
    int currentX = xStart;
    ...
    for(i = 0; i < moduleCount; i++)
    {
		buildingType[i] = getRandom(3);     //Change this to modify the accessible modules
        xBuilding[i] = currentX;
        currentX += modules[buildingType[i]]->size.x + 10 + buildingType[i];
    }
...
{
    const int moduleCountPrevious = getRandomInRange(1, 3), moduleCountAfter = getRandomInRange(1, 3);
...
void generateBuilding(Level* level, Module* module, int xBuilding, bool mirror)
{
    ...
    pasteModule(module, level, asPoint(xBuilding + module->ref.x, getTerrainHeight(level->terrain, xBuilding)), false, mirror);
}
...
void generateBuildings(Level* level)
{
    ...
    {
        moduleSelection = getRandomInRange(0, 100);
        if(moduleSelection < 50 && currentX < PLAINS_LIMIT)        //50% chance
        {
            currentX = genSmallVillage(level, modules, currentX) + getRandomInRange(60, 100);
        }
        else if(moduleSelection < 80 && currentX < PLAINS_LIMIT)   //30% chance
        {
            if(currentX > PLAINS_LIMIT)
                currentX = genCastle(level, modules, currentX) + getRandomInRange(100, 180);
            else
                currentX = genVillage(level, modules, currentX) + getRandomInRange(100, 180);
        }
        else                            //20% chance
        {
            if(currentX > PLAINS_LIMIT)
                currentX = genCastle(level, modules, currentX) + getRandomInRange(100, 200);
            else
                currentX = genVille(level, modules, currentX) + getRandomInRange(100, 200);
        }
    }
