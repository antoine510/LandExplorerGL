...
static void processHit(Entities* entities, int id, int damage, float knockback, Vect2 direction);
static void groundSlowing(Entities* entities, int id);
static void oneBlocHop(Terrain* terrain, Entities* entities, unsigned int id, char hCol, Vect2 v);

static void processTerrainCols(Terrain* terrain, Entities* entities, unsigned int id, char vCol, char hCol, Vect2 v);
...
void checkTerrainCols(Terrain* terrain, Entities* entities)
{
    ...
    for(i = 0; i < ENTITY_COUNT; i++)
    {
        ...
        {
            Vect2* pos = &entities->collData[i].pos;
            ...
            if(vCollision || hCollision) processTerrainCols(terrain, entities, i, vCollision, hCollision, (Vect2){vx, vy});
...
void processHit(Entities* entities, int id, int damage, float knockback, Vect2 direction)
{
    if(entities->collData[id].collTime > COLL_HIT_DELAY * PCPS)
    {
        entities->status[id].HP -= damage;
		multiplyBy(&direction, knockback);
        addTo(&entities->dynData[id].velocity, direction);
            ...
            case COLLTYPE_PLAYER:
                ...
                case COLLTYPE_WEAPON:
                    if(entities->weaponData[slaveID].parentID != masterID)
                    {
                        ...
                        case weapon_arrow:
                            processHit(entities, masterID, entities->weaponData[slaveID].damage,
                                   entities->weaponData[slaveID].knockback, vect2_normalize(entities->dynData[slaveID].velocity));
...
Vect2 processHitDirection(Entities* entities, int masterID, int slaveID)
{
    Vect2 hitDirection = {entities->collData[masterID].pos.x + entities->collData[masterID].sz.x/2 -
    ...
    return vect2_normalize(hitDirection);
}
...
{
    return vect2_norm((Vect2){processXDistance(entities, id1, id2), processYDistance(entities, id1, id2)});
}

//The entity is Dynamic and Collideable
void processTerrainCols(Terrain* terrain, Entities* entities, unsigned int id, char vCol, char hCol, Vect2 v)
{
    switch(entities->collData[id].collType)
    {
    case COLLTYPE_PLAYER:
...
void oneBlocHop(Terrain* terrain, Entities* entities, unsigned int id, char hCol, Vect2 v)
{
    Vect2* pos = &entities->collData[id].pos;
