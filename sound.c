...
static bool loadSound(Sound* sound, const char* path, unsigned int index);
static bool loadMusic(Sound* sound, const char* path, unsigned int index);
...
Sound* sound_create()
{
    ...
    soundStack = calloc(1, sizeof(SoundStack));
    initSoundstack();
...
bool loadSounds(Sound* sound)
{
	...
	while (soundNode)
    {
		...
		{
		    char filename[MAX_FILENAME_SIZE] = "audio/";
            strncat(filename, asStringl(soundType, "path"), MAX_FILENAME_SIZE);
            strcat(filename, asStringl(soundNode, "path"));
	...
	while (musicNode)
    {
		...
		{
		    char filename[MAX_FILENAME_SIZE] = "audio/";
            strcat(filename, asStringl(soundType, "path"));
            strcat(filename, asStringl(musicNode, "path"));
...
bool loadSound(Sound* sound, const char* path, unsigned int index)
{
    FMOD_RESULT result = FMOD_System_CreateSound(sound->system, path, FMOD_CREATESAMPLE, 0, &sound->sounds[index]);
    if(result != FMOD_OK) {fprintf(stderr, "FMOD error : Couldn't load sound : %s\n", path); return true;}
    return false;
}

bool loadMusic(Sound* sound, const char* path, unsigned int index)
{
    FMOD_RESULT result = FMOD_System_CreateSound(sound->system, path, FMOD_CREATESTREAM | FMOD_LOOP_NORMAL, 0, &sound->musics[index]);
    if(result != FMOD_OK) {fprintf(stderr, "FMOD error : Couldn't load music : %s\n", path); return true;}
